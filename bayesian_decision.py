...
LastEditTime: 2024-05-15 14:45:02
FilePath: /digital_image_processing/bayesian_decision.py
Description: a bayesian method to make some front candidate to be fronts (Ping, 2014)
...
def bayesian(marked_matrix, p10, p20, gradient_magnitude, SST):
    ...
    likelihood_fornt_matrix = np.zeros_like(marked_matrix)
    likelihood_nonfront_matrix = np.zeros_like(marked_matrix)
    
    for i in range(1, rows-1):
        for j in range(1, cols-1):
            if np.isnan(marked_matrix[i,j]):
                prior_matrix[i, j] = np.nan
                likelihood_fornt_matrix[i, j] = np.nan
                likelihood_nonfront_matrix[i, j] = np.nan
            elif marked_matrix[i, j] == 2:
    ...
    for i in range(1, rows-1):
        for j in range(1, cols-1):
            if marked_matrix[i, j] == 2:
                ...
                likelihood_nonfront_matrix[i, j] = p_x_w2
    ...
    return marked_matrix, LDE, BD, prior_matrix, likelihood_fornt_matrix, likelihood_nonfront_matrix